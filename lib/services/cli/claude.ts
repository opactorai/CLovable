...
export async function executeClaude(
  ...
  const enableDeepThinking = options.enableDeepThinking === true;
  const configuredThinkingTokens = Number(process.env.CLAUDE_CODE_MAX_THINKING_TOKENS);
  const maxThinkingTokens = enableDeepThinking && Number.isFinite(configuredThinkingTokens) && configuredThinkingTokens > 0
  ...
  try {
    ...
    if (enableDeepThinking) {
      console.log(`[ClaudeService] 🧠 Deep thinking enabled (max thinking tokens: ${maxThinkingTokens ?? 'default'})`);
    }
