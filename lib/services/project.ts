...
export async function updateProjectCliPreference(
  ...
  const targetCli = input.preferredCli ?? existing?.preferredCli ?? 'claude';
  const normalizedSelectedModel = input.selectedModel === null
    ? null
  ...
  const result = await prisma.project.update({
    ...
    data: {
      ...
      ...(typeof normalizedSelectedModel === 'string' && normalizedSelectedModel.length > 0
        ? { selectedModel: normalizeModelId(targetCli, normalizedSelectedModel) }
        : normalizedSelectedModel === '' || normalizedSelectedModel === null
        ? { selectedModel: null }
        : {}),
      updatedAt: new Date(),
    },
