...
const CLI_OPTIONS: CLIOption[] = [
  ...
  },
  {
    ...
    name: 'OpenCode',
    icon: '🧰',
    description: 'Open-source terminal agent with MCP integration',
    color: 'from-gray-800 to-slate-700',
    downloadUrl: 'https://opencode.ai',
    installCommand: 'npm i -g opencode-ai',
    models: [],
    features: ['MCP servers (local/remote)', 'Provider-agnostic routing', 'Local + remote models'],
  },
];
...
export default function CreateProjectModal({ open, onClose, onCreated, onOpenGlobalSettings }: CreateProjectModalProps) {
    ...
    try {
      ...
      if (settings) {
        ...
        if (preferredModelSetting) {
          ...
          const targetCli = effectiveCLIs.find((cli) => cli.id === preferredCLI);
          const fallbackModelId = targetCli?.models?.[0]?.id;
          setSelectedModel(fallbackModelId ? sanitizeModel(preferredCLI, fallbackModelId) : '');
        }
      } else {
        ...
        const fallbackModelId = effectiveCLIs[0]?.models?.[0]?.id;
        setSelectedModel(fallbackModelId ? sanitizeModel(fallbackCLI, fallbackModelId) : '');
      ...
      setSelectedCLI(fallbackCLI);
      const fallbackModelId = available[0]?.models?.[0]?.id;
      setSelectedModel(fallbackModelId ? sanitizeModel(fallbackCLI, fallbackModelId) : '');
      setFallbackEnabled(true);
    }
  }, []);
  ...
  const hasModelOptions = Boolean(selectedCLIOption?.models?.length);
  const selectedModelLabel = selectedModelOption?.name
    ?? (hasModelOptions
      ? 'Select a model'
      : (selectedCLI === 'opencode'
          ? 'Auto (configure via OpenCode config)'
  ...
  async function submit() {
    ...
    const cliDefinition = CLI_OPTIONS.find(cli => cli.id === finalCLI) ?? enabledCLIs.find(cli => cli.id === finalCLI);
    const requiresModel = Boolean(cliDefinition?.models.length);

    if (!requiresModel) {
      finalModel = '';
    }

    if (!finalCLI || (requiresModel && !finalModel)) {
    ...
    if (finalModel) {
      projectData.selectedModel = finalModel;
      projectData.cli_settings = {
        [finalCLI]: {
          model: finalModel,
        },
      };
                ...
                <div className="relative">
                  ...
                  {hasModelOptions ? (
                    ...
                    <div className="p-4 bg-gray-50 border border-dashed border-gray-300 rounded-lg text-sm text-gray-600">
                      OpenCode selects the model based on its own configuration. Edit <code className="font-mono">.opencode/opencode.jsonc</code> to customize.
                    </div>
                  )}
                </div>
              </div>
