...
const CLI_OPTIONS: CLIOption[] = [
  {
    ...
    models: CLAUDE_MODEL_DEFINITIONS.map(({ id, name, description }) => ({ id, name, description })),
...
export default function ProjectSettings({ isOpen, onClose, projectId, projectName, initialTab = 'general' }: ProjectSettingsProps) {
  ...
  const [globalSettings, setGlobalSettings] = useState<GlobalSettings>({
    ...
    cli_settings: {
      claude: { model: CLAUDE_DEFAULT_MODEL, enabled: true },
  ...
  const loadProjectInfo = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        const preferredCli = project.preferredCli ?? project.preferred_cli ?? 'claude';
        const selectedModelRaw = project.selectedModel ?? project.selected_model;
        const fallbackEnabled = project.fallbackEnabled ?? project.fallback_enabled ?? false;
        const normalizedModel = selectedModelRaw
          ? normalizeClaudeModelId(selectedModelRaw)
        ...
        setProjectCLIInfo({
          currentPreference: preferredCli,
          currentModel: getClaudeModelDisplayName(normalizedModel),
          fallbackEnabled,
        });
      }
    } catch (error) {
  ...
  const loadGlobalSettings = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data?.cli_settings) {
          for (const [cli, config] of Object.entries(data.cli_settings)) {
  ...
  }, [isOpen, projectId, loadProjectInfo, loadServiceConnections, loadGlobalSettings, checkCLIStatus]);
                    ...
                    {projectCLIInfo ? (
                      <div className="p-4 rounded-xl border-2 border-gray-900 dark:border-white bg-gray-50 dark:bg-gray-700">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 flex items-center justify-center">
                              <span className="text-xl">
                                {CLI_OPTIONS.find(cli => cli.id === projectCLIInfo.currentPreference)?.icon || '🤖'}
                              </span>
                            </div>
                            <h3 className="font-semibold text-gray-900 dark:text-white">
                              {CLI_OPTIONS.find(cli => cli.id === projectCLIInfo.currentPreference)?.name || 'Unknown CLI'}
                            </h3>
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          ...
                          {projectCLIInfo.currentModel && (
                            <div className="mt-1">
                              Current Model: <span className="font-medium">{projectCLIInfo.currentModel}</span>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`w-12 h-6 rounded-full transition-colors ${
                        projectInfo?.fallbackEnabled ? 'bg-gray-900 dark:bg-white' : 'bg-gray-300 dark:bg-gray-600'
                      }`}
                    >
                      <div
                        className={`w-4 h-4 bg-white rounded-full transition-transform ${
                          projectInfo?.fallbackEnabled ? 'translate-x-7' : 'translate-x-1'
                        }`}
                      />
                    </div>
                  </div>
