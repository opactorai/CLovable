...
function ensureStandaloneArtifacts() {
  ...
  if (!fs.existsSync(serverPath)) {
    throw new Error(
      'Next.js standalone 서버 파일이 없습니다. `npm run build` 후 다시 시도하세요.'
    );
  }
  return serverPath;
}
...
async function startProductionServer() {
  ...
  nextServerProcess.on('exit', (code, signal) => {
    if (!shuttingDown && typeof code === 'number' && code !== 0) {
      console.error(`⚠️  Next.js 서버가 코드 ${code} (signal: ${signal ?? 'n/a'})로 종료되었습니다.`);
    }
    nextServerProcess = null;
  });

  await waitForUrl(url).catch((error) => {
    console.error('❌ Next.js 프로덕션 서버가 기동되지 못했습니다.');
    throw error;
  });
...
app.on('activate', () => {
  if (BrowserWindow.getAllWindows().length === 0) {
    createMainWindow().catch((error) => {
      console.error('❌ 메인 윈도우를 다시 생성하지 못했습니다.');
...
if (setupSingleInstanceLock()) {
  app
    ...
    .catch((error) => {
      console.error('❌ Electron 앱 초기화 중 오류가 발생했습니다.');
      console.error(error instanceof Error ? error.stack || error.message : error);
      app.quit();
    });
}
