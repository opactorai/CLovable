...
- **OpenCode** - Open-source terminal agent with MCP + local model support
...
### OpenCode (Local + MCP)
**[OpenCode](https://opencode.ai)** - Open-source CLI agent focused on local workflows and MCP tooling
- **Features**: Provider-agnostic model routing, first-class MCP server integration (local & remote), lightweight TUI/JSON streaming
- **Context**: Determined by your configured provider/model (e.g. LM Studio, Ollama, Bedrock, etc.)
- **Pricing**: Completely free and open-source
- **Installation**:
  ```bash
  npm i -g opencode-ai
...
Each project can include an optional `.opencode/opencode.jsonc` file—now editable from the project settings UI—to point OpenCode at local model endpoints or MCP servers. Leave the file empty to let OpenCode auto-discover what is installed on your machine.
