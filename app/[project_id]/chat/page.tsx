...
export default function ChatPage() {
  ...
  const updateSelectedModel = useCallback((model: string | null, cliOverride?: string) => {
...
const persistProjectPreferences = useCallback(
  async (changes: { preferredCli?: string; selectedModel?: string | null }) => {
    if (!projectId) return;
  ...
  const handleModelChange = useCallback(
    async (option: ModelOption, opts?: { skipCliUpdate?: boolean; overrideCli?: string }) => {
      ...
      try {
        ...
        const shouldLogSwitchMessage = !(
          targetCli === 'opencode' && (!sanitizedModelId || sanitizedModelId.length === 0)
        );

        if (shouldLogSwitchMessage) {
          try {
  ...
  const handleCliChange = useCallback(
    async (cliId: string) => {
      ...
      try {
        ...
        const defaultModelRaw = getDefaultModelForCli(cliId);
        const defaultModel = defaultModelRaw && defaultModelRaw.length > 0 ? defaultModelRaw : null;
  ...
  useEffect(() => {
    ...
    const hasModelsForPreferredCli = modelOptions.some(option => option.cli === preferredCli);
    if (!hasModelsForPreferredCli) {
      return;
    }
